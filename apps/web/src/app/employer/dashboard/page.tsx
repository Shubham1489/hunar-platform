...
const MOCK_STATS = [
  { label: 'Active Jobs', value: '8', icon: Briefcase, color: '#3B82F6', bg: 'rgba(59,130,246,0.1)', change: '+2 this week' },
  { label: 'Total Applicants', value: '156', icon: Users, color: '#F97316', bg: 'rgba(249,115,22,0.1)', change: '+23 today' },
  { label: 'Hired This Month', value: '12', icon: CheckCircle, color: '#10B981', bg: 'rgba(16,185,129,0.1)', change: '67% fill rate' },
  { label: 'Avg Match Score', value: '87%', icon: Sparkles, color: '#7C3AED', bg: 'rgba(124,58,237,0.1)', change: 'AI powered' },
];
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--surface-1)' }}>
      ...
      <aside style={{ width: 260, background: 'white', padding: '24px 16px', display: 'flex', flexDirection: 'column', boxShadow: 'var(--shadow-sm)' }}>
        ...
        <div style={{ background: 'linear-gradient(135deg, #F97316, #FBBF24)', borderRadius: 16, padding: 20, marginBottom: 24, color: 'white' }}>
          <p style={{ fontWeight: 700, fontSize: 15, marginBottom: 2 }}>BuildCraft Infra</p>
          <p style={{ fontSize: 12, opacity: 0.8 }}>Construction & Infrastructure</p>
      ...
      <main style={{ flex: 1, padding: 32, overflow: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32 }}>
          <div>
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: 15 }}>23 new applicants since yesterday</p>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 20, marginBottom: 32 }}>
          {MOCK_STATS.map(({ label, value, icon: Icon, color, bg, change }) => (
            <div key={label} className="card" style={{ padding: 24 }}>
              ...
              <p style={{ fontSize: 12, marginTop: 8, color: 'var(--text-tertiary)' }}>{change}</p>
            </div>
          ))}
        </div>
                ...
                <div key={job.id} className="card" style={{ padding: 20 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                    <div>
                      ...
                      <p style={{ color: 'var(--text-tertiary)', fontSize: 13 }}>
                        <Clock size={11} style={{ display: 'inline', verticalAlign: 'middle' }} /> Posted {job.posted}
                      </p>
                    </div>
                    <span style={{
                  ...
                  <div style={{ display: 'flex', gap: 24 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      ...
                      <span style={{ fontSize: 13, color: 'var(--text-secondary)' }}>{job.applicants} applicants</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <CheckCircle size={14} color="#10B981" />
                      <span style={{ fontSize: 13, color: 'var(--text-secondary)' }}>{job.hired} hired</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <Eye size={14} color="var(--text-tertiary)" />
                      <span style={{ fontSize: 13, color: 'var(--text-secondary)' }}>{job.views} views</span>
                    </div>
                  </div>
                </div>
              ))}
